  ...
  try {
    const source = await prisma.$transaction(async (tx) => {
      ...
      const changed = current.status !== nextStatus || current.permissionNote !== nextNote || current.allowImport !== allowImport ||
        current.allowExtract !== allowExtract || current.allowEvidenceText !== allowEvidenceText || current.allowRelate !== allowRelate || current.allowExport !== allowExport || current.retentionDays !== retentionDays ||
      ...
      const updated = await tx.source.update({ where: { id }, data: {
        permissionNote: nextNote, status: nextStatus, allowImport, allowExtract, allowEvidenceText, allowRelate, allowExport, retentionDays,
        policyVersion: version, policyChangedById: user.id, expiresAt,
      } });
      await tx.sourcePolicySnapshot.create({ data: {
        sourceId: id, version, status: nextStatus, allowImport, allowExtract, allowEvidenceText, allowRelate, allowExport, retentionDays, expiresAt,
