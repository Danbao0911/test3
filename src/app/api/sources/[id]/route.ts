  ...
  try {
    const source = await prisma.$transaction(async (tx) => {
      ...
      const updated = await tx.source.update({ where: { id }, data: {
        permissionNote: parsed.data.permissionNote, status: nextStatus,
        allowImport: nextStatus === "APPROVED" ? parsed.data.allowImport ?? current.allowImport : false,
        allowExtract, allowEvidenceText, retentionDays: parsed.data.retentionDays,
        policyVersion: { increment: 1 }, policyChangedById: user.id,
