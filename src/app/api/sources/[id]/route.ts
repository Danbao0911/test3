...
export async function PATCH(request: Request, { params }: Context) {
  ...
  try {
    const source = await prisma.$transaction(async (tx) => {
      ...
      const allowExport = nextStatus === "APPROVED" && (parsed.data.allowExport ?? current.allowExport);
      const allowedExportFields = parsed.data.allowedExportFields ?? current.allowedExportFields;
      ...
      const changed = current.status !== nextStatus || current.permissionNote !== nextNote || current.allowImport !== allowImport ||
        current.allowExtract !== allowExtract || current.allowEvidenceText !== allowEvidenceText || current.allowRelate !== allowRelate || current.allowExport !== allowExport || JSON.stringify(current.allowedExportFields) !== JSON.stringify(allowedExportFields) || current.retentionDays !== retentionDays ||
      ...
      const updated = await tx.source.update({ where: { id }, data: {
        permissionNote: nextNote, status: nextStatus, allowImport, allowExtract, allowEvidenceText, allowRelate, allowExport, allowedExportFields, retentionDays,
        policyVersion: version, policyChangedById: user.id, expiresAt,
      } });
      await tx.sourcePolicySnapshot.create({ data: {
        sourceId: id, version, status: nextStatus, allowImport, allowExtract, allowEvidenceText, allowRelate, allowExport, allowedExportFields: JSON.stringify(allowedExportFields), retentionDays, expiresAt,
