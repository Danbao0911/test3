...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ item: { id: job.id, downloadUrl: `/api/exports/${job.id}/download?token=${encodeURIComponent(job.token)}`, expiresAt: job.expiresAt, rowCount: job.rowCount, excludedCount: job.excludedCount } }, { status: 201 });
  } catch (error) {
