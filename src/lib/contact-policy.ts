...
export function contactUsable(contact: { status: string; expiresAt: Date; ownershipConfirmed: boolean; businessConfirmed: boolean; reviewedAt: Date | null; suppressed?: boolean; evidence: { policyVersion: number; source: ContactPolicy; policySnapshot?: { isLegacy: boolean } } }, now = new Date()) {
  return contact.status === "APPROVED" && contact.ownershipConfirmed && contact.businessConfirmed &&
    ...
    contact.evidence.policySnapshot?.isLegacy !== true && contact.suppressed !== true;
}
