...
test("T06 账号关联只能人工确认，确认后仍可按来源策略计算可用性", async ({ page }) => {
  ...
  await expect(page.locator(`[data-link-id="${candidateData.item.id}"]`)).toContainText("已确认");
  await expect(page.locator(`[data-link-id="${candidateData.item.id}"]`)).toContainText("当前可用");
});
