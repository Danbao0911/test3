...
test("T07 管理员导出前检查策略并生成一次性 CSV 链接", async ({ page }) => {
  ...
  const approved = await page.request.patch(`/api/sources/${sourceId}`, { headers: { Origin: origin }, data: { expectedPolicyVersion: source.item.policyVersion, status: "APPROVED", allowImport: true, allowExtract: true, allowEvidenceText: true, allowExport: true, allowedExportFields: ["DISPLAY_NAME", "PLATFORM", "ORGANIZATION", "CONTACT_TYPE", "CONTACT_VALUE", "SOURCE_URL", "REVIEWED_AT"] } });
