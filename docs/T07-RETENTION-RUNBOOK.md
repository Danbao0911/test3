...
维护命令现在输出 `scanComplete`、`enforcementComplete` 和 `complete`。扫描结束但存在未知规则、缺失 key、未知算法、未知 scope 或不完整指纹时，不能作为恢复完成：`replay` 返回退出码 3；达到 `--max-batches` 且仍有游标时返回退出码 2。正常完成和只读 dry-run 返回退出码 0。

`--checkpoint` 使用签名的、绑定操作类型、数据库名称、`RETENTION_RUN_ID`/`TEST_RUN_ID` 和截止时间的续跑凭据。命令输出的 `continuation` 可直接传给新的进程；不得手工修改其中的游标。服务端配置 `RETENTION_CHECKPOINT_KEY`，没有该配置时仅允许隔离非生产模式使用测试派生值。
...
重放前按算法版本和 key id 选择规则：当前 v2 规则和具有可信 key 映射的历史 `legacy-v1` 抑制指纹可兼容匹配。生产环境应通过 `SUPPRESSION_HMAC_KEY_ID`、`SUPPRESSION_HMAC_KEY` 和受控的 `SUPPRESSION_HMAC_KEYS_JSON` 提供真实 key 映射；缺少旧 key、未知版本、未知 scope、空指纹或不完整身份字段只报告 `blockedRules`，不静默放行，也不把当前 key 冒充旧 key。恢复副本必须先导入备份时点之后产生的删除/抑制规则；旧 UUID 只作为历史审计，不能自行识别新 UUID；规则不会保存原始联系人作为黑名单。
