...
2. 创建带唯一标识的虚构账号、证据和联系方式，做一次 `pg_dump` 基线。
3. 通过生产服务 `deleteTarget` 产生删除/抑制规则并删除基线对象，再只导出规则元数据。
4. 创建临时恢复库，恢复基线和规则，调用真实 `replayDeletionRules`。
5. 查询数据库确认账号、联系人均不可用；重复重放确认删除数为 0 且任务完成。
6. 断开连接后删除本次临时恢复库、删除本轮 fixture 和临时 dump。

脚本不接受生产数据库、不接受外部主机、不把原始 fixture 值写入 stdout；工具失败时只返回退出码和不含数据库工具原文的错误。若在 `--max-batches` 场景使用既有 `retention:replay`，输出 `incomplete` 和 checkpoint 时必须保存 checkpoint，并用同一 run ID 继续；不能把累计删除数当成扫描完成。
