...
T10 已补齐隔离运行预检、真实 PostgreSQL 备份/恢复/删除规则重放演练、E2E 成功截图/录像 artifact、预算 hard stop 和部署/回滚/告警手册。最终实现与测试 SHA 为 `d4d9ea54a8eb7b39b940ab40ee201d3bdcad15b9`；Push/PR CI 均通过，unit 136、integration 69、E2E 10，失败/跳过均为 0。生产备份存储、告警通道、调度、真实数据闭环及平台 API 仍未部署或验证，整体项目继续按“部分完成”处理。
...
当前“有效天数”限制可用性，并由 T07 到期清理任务在隔离环境物理清理过期联系、导出和抑制。输入证据不会自动验证页面所有权；人工确认仍是必要步骤。规则较保守，未识别信息不等于不存在。联系失效/驳回当前不可逆，同一策略版本不会自动刷新过期证据；修正需新来源策略版本和新取得的证据。T05 工作台、T06 关联审核、T07 受控导出/删除/抑制和 T10 隔离恢复演练已交付；四平台真实采集、生产告警和生产外联仍保持关闭。
...
推荐前向修复。不得把旧代码直接对外运行在新多角色库上，旧版本没有角色检查。紧急回滚应停服务、保存当前证据及审核记录，恢复升级前已验证的整库备份和匹配代码；恢复会丢失备份后的写入，须由负责人明确授权。不提供自动删表降级命令。T10 已在 CI 专用库完成备份/恢复/重放演练，但未对生产库执行。
